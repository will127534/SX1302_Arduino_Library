...
int sx125x_reg_w(radio_reg_t idx, uint8_t data, uint8_t rf_chain) {
    ...
    if ((reg.leng == 8) && (reg.offs == 0)){
        /* direct write */
        com_stat = sx125x_com_w(lgw_com_cs(), ((rf_chain == 0) ? LGW_SPI_MUX_TARGET_RADIOA : LGW_SPI_MUX_TARGET_RADIOB), reg.addr, data);
    } else {
        /* read-modify-write */
        com_stat = sx125x_com_r(lgw_com_cs(), ((rf_chain == 0) ? LGW_SPI_MUX_TARGET_RADIOA : LGW_SPI_MUX_TARGET_RADIOB), reg.addr, &r);
        mask = ((1 << reg.leng) - 1) << reg.offs;
        w = (r & ~mask) | ((data << reg.offs) & mask);
        com_stat |= sx125x_com_w(lgw_com_cs(), ((rf_chain == 0) ? LGW_SPI_MUX_TARGET_RADIOA : LGW_SPI_MUX_TARGET_RADIOB), reg.addr, w);
    }
