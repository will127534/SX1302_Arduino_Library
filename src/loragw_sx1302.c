...
#include "cal_fw.h" /* text_cal_sx1257_16_Nov_1 */
...
int sx1302_lora_syncword(bool ispublic, uint8_t lora_service_sf) {
    int err = LGW_REG_SUCCESS;
    ...
    if ((ispublic == false) || (lora_service_sf == DR_LORA_SF5) || (lora_service_sf == DR_LORA_SF6)) {
...
int sx1302_agc_load_firmware(const uint8_t *firmware) {
    int32_t val;
    uint8_t firmware_backup[MCU_FW_SIZE];
    uint8_t fw_check[MCU_FW_SIZE];
    int err = LGW_REG_SUCCESS;
    memcpy(firmware_backup,firmware,MCU_FW_SIZE);
    ...
    if (memcmp(firmware_backup, fw_check, sizeof fw_check) != 0) {
